            ...
            if self.args.batch_norm_round_0 and r == 0:
                with torch.no_grad():
                    if self.args.mixed_precision:
                        with autocast():
                            dict_calc_losses, outputs = self.calc_loss_and_output(images, labels, conditions)
                    else:
                        dict_calc_losses, outputs = self.calc_loss_and_output(images, labels, conditions)
                optimizer.zero_grad()
            else:
    ...
    def test(self, metric, swa=False):
        ...
        with torch.no_grad():
            for i, (images, labels, conditions) in enumerate(self.loader):
                ...
                # Perform the conditions mapping
                ...
                try:
                    outputs = self.get_test_output(images, conditions)
                except AttributeError:
                    outputs = self.get_test_output(images)["out"]
