...
class TargetDataset(data.Dataset, metaclass=ABCMeta):
    ...
    def __init__(self, paths, root, transform=None, test_transform=None, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
        ...
        if self.weather_map is not None:
            ...
            for i, img in enumerate(self.paths['x']):
                cur_weather = self.weather_map[img.split('/')[1]]
    ...
    def __getitem__(self, index):
        ...
        if self.classes is not None:
            ...
            weather = "none"
        condition = f"{self.category}_{weather}"
        ...
        if self.test:
            ...
            if self.hp_filtered:
                x, x_hpf = x
                return (plot_x, x, x_hpf), y, condition
            return (plot_x, x), y, condition

        out = self._apply_train_transform(x, y, x_hpf)
        return out[0], out[1], condition
    ...
    def _apply_train_transform(self, x, y, x_hpf=None):
        ...
        return [self.transform(x), torch.empty(0)]
