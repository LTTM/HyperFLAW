...
class FLYAWARES(SourceDataset):
    ...
    dataset_dir = Path("FLYAWARE-S/renders")
    split_dir = Path(__file__).parent.parent.parent.parent / "data/flyaware-s/splits"
    ...
    def __init__(
        ...
        for dts in target_dataset:
            assert (
                dts in FLYAWARES.labels2train
            ), f"Class mapping missing for {target_dataset}, choose from: {FLYAWARES.labels2train.keys()}"
        if len(target_dataset) > 1:
            self.labels2train = FLYAWARES.labels2train['cityscapes']
        else:
            self.labels2train = FLYAWARES.labels2train[target_dataset[0]]
        ...
        for cond in self.weather_conditions:
            with open(syndrone_splits[cond]) as f:
                lines = f.read().splitlines()
                classes[cond] += range(last_len, len(lines) + last_len)
                last_len += len(lines)
                syndrone_data += [
                    {
                        f"syndrone_{cond}": f'{self.dataset_dir}/{l.split(" ")[0]} {self.dataset_dir}/{l.split(" ")[1]}'
                    }
                    for l in lines
                ]
        self.paths = syndrone_data
        self.classes = classes

    def __getitem__(self, index):
        transform = self.transform if not self.test else self.test_transform
        condition = f"{self.category}_{list(self.paths[index].keys())[0].split('_')[1]}"
