...
from .args import SOURCE_DATASETS, TARGET_DATASETS

DATASET_DIR_NAME = {
    "acdc": "HyperFLAW-ACDC",
    "cityscapes": "cityscapes",
    "flyawarer": "FLYAWARE-R",
    "flyawarerxl": "FLYAWARE-R-XL",
}
...
class DatasetHandler(object):
    ...
    def __init__(self, args, writer):
        ...
        if args.additional_test_datasets != '':
            self.__additional_test_datasets_init()
    ...
    def __source_dataset_init(self):
        ...
        try:
            train_ds = dataset(root=self.args.root, transform=train_transform, test_transform=test_transform,
                            split='train', only_clear = self.args.only_clear)
        except TypeError:
            train_ds = dataset(root=self.args.root, transform=train_transform, test_transform=test_transform,
                            split='train')
        test_ds = dataset(root=self.args.root, transform=train_transform, test_transform=test_transform,
                          split='test')

        self.source_stats = {'mean': train_ds.mean, 'std': train_ds.std}

        self.clients_args['train'].append({'client_id': 'source_train', 'dataset': train_ds})
        self.clients_args['test'].append({'client_id': 'source_test', 'dataset': test_ds})

    def __additional_test_datasets_init(self):
        if self.args.additional_test_datasets:
            datasets_list = [self.args.additional_test_datasets] if "," not in self.args.additional_test_datasets \
                else self.args.additional_test_datasets.split(",")

            for i_dataset in datasets_list:
                train_transform, test_transform, dataset = \
                    get_dataset(self.args.model, i_dataset, target_dataset=self.args.target_dataset,
                                cv2=self.args.cv2, random_flip=self.args.random_flip, color_jitter=self.args.color_jitter,
                                gaussian_blur=self.args.gaussian_blur)

                if i_dataset in SOURCE_DATASETS:
                    test_ds = dataset(root=self.args.root, transform=train_transform, test_transform=test_transform,
                                      split='test')
                else:
                    train_data_dir, test_data_dir = self.__get_paths(self.args.root, DATASET_DIR_NAME[i_dataset],
                                                                     self.args.clients_type)
                    _, test_data = self.__read_target_data(train_data_dir, test_data_dir)
                    test_users = test_data.keys()
                    for i, user in enumerate(test_users):
                        test_ds = self.__gen_ds(test_data[user], i_dataset, dataset, train_transform, test_transform,
                                           split="test", hp_filtered=self.args.hp_filtered)

                self.clients_args['test'].append({'client_id': f'add_test_{i_dataset}', 'dataset': test_ds})

    def __gen_ds(self, paths, dataset_name, dataset, train_transform, test_transform, split='train', hp_filtered=False):
        if dataset_name == 'cityscapes':
            return dataset(paths, root=self.args.root, transform=train_transform, test_transform=test_transform,
        ...
        if dataset_name in ['acdc', 'flyawarer', 'flyawarerxl', 'flyawarer_acdc', 'flyaware_acdc']:
            return dataset(paths, root=self.args.root, transform=train_transform, test_transform=test_transform,
                                       hp_filtered=hp_filtered)
        raise NotImplementedError

    def __target_dataset_init(self):
        for dts in self.args.target_dataset:
            dataset_name = dts
            train_data_dir, test_data_dir = self.__get_paths(self.args.root, DATASET_DIR_NAME[dataset_name], self.args.clients_type)
...
def color_map(dataset):
    ...
    if dataset == ['cityscapes'] or dataset == ['acdc'] or sorted(dataset) == ['acdc', 'flyawarer'] or sorted(dataset) == ['acdc', 'flyawarerxl']:
        color = [(128, 64, 128), (244, 35, 232), (70, 70, 70), (102, 102, 156), (190, 153, 153), (153, 153, 153),
                 (250, 170, 30), (220, 220, 0), (107, 142, 35), (152, 251, 152), (70, 130, 180), (220, 20, 60),
                 (255, 0, 0), (0, 0, 142), (0, 0, 70), (0, 60, 100), (0, 80, 100), (0, 0, 230), (119, 11, 32),
                 (0, 0, 0)]
        n = 256
    elif dataset == ['flyawarer']:
        color = [( 70, 70, 70), (128, 64,128), (  0,  0,142), (107,142, 35), (220, 20, 60), (  0,  0,  0)]
        n = 256
    elif dataset == ['flyawarerxl']:
        color = [( 70, 70, 70), (128, 64,128), (  0,  0,142), (107,142, 35), (220, 20, 60), (  0,  0,  0)]
        n = 256
    else:
        raise NotImplementedError
