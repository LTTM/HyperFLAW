...
class FdaServer(OracleServer):
    ...
    def save_pretrained_server(self, model_state, best: bool):
        ...
        ckpt_path = Path(__file__).parent.parent.parent.parent / 'checkpoints' / self.args.framework / \
                    self.args.source_dataset / self.args.target_dataset / \
                    f'pretrained_server_{self.writer.wandb.id}_{"_best" if best else ""}.ckpt'
    ...
    def train_source(self, train_clients, test_clients, train_metric, test_metric, optimizer, scheduler,
        ...
        for e in range(0, num_source_epochs):
            ...
            if (e + 1) % self.args.server_test_interval == 0 or (e + 1) == num_source_epochs:
                _, _ = test_fn(test_clients["source"], test_metric, e, 'EPOCH', max_scores,
                                                    cl_type='source')

                max_scores, found_new_max = test_fn(test_clients["target"], test_metric, e, 'EPOCH', max_scores, cl_type='target')
                if found_new_max:
